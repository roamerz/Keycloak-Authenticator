...
public class IPAuthenticator implements Authenticator {
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        if
            (!allowedIPAddress.contains(remoteIPAddress)) {
            logger.infof("IPs do not match. Realm %s expected one of the following IP addresses %s but user %s logged from %s", realm.getName(), allowedIPAddress, user.getUsername(), remoteIPAddress);
            user.setAttribute(IP_BASED_OTP_CONDITIONAL_USER_ATTRIBUTE, Collections.singletonList("force"));
        } else {
            logger.infof("IPs match. Realm %s expected one of the following IP addresses %s and user %s logged from %s", realm.getName(), allowedIPAddress, user.getUsername(), remoteIPAddress);
